...
// https://github.com/YosysHQ/picorv32/blob/main/firmware/sieve.c
...
void print_chr(char ch) { uart_put_char(ch); }
void print_str(char *p) { uart_put_string(p); }
...
void print_hex(unsigned int val, int digits)
{
    for (int i = (4*digits)-4; i >= 0; i -= 4) {
        unsigned int dd = "0123456789ABCDEF"[(val >> i) % 16];
        uart_put_char(dd);
    }
}
...
{
    if ((bitmap[idx/32] & (1 << (idx % 32))) != 0) return 1;
    else return 0;
}
