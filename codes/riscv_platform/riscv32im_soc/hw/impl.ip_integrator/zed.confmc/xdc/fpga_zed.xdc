...
#--------------------------------------------------------
# GPIO Output [7:0] -> ZedBoard 8개 LED (LD0-LD7) - Bank 33 (3.3V 고정)
# 주의: Block Design에서 gpio_out은 8비트만 정의되어 있음
# 주의: 현재 Block Design에서 gpio_out/gpio_in 포트가 제거되었으므로 주석 처리
# 필요시 Block Design에서 포트를 다시# LED (8 개, gpio_out[7:0])
...
# NOTE: gpio_out[15:8] are used internally:
#   gpio_out[8]  -> spi_cs_n  (connected internally in RTL)
#   gpio_out[9]  -> spi_sck   (connected internally in RTL)
#   gpio_out[10] -> spi_mosi  (connected internally in RTL)
#   gpio_out[11] -> spi_rst   (connected internally in RTL)
#   gpio_out[15:12] -> keypad_col (connected internally in RTL)
# These internal signals are then routed to external pins defined below (spi_*, keypad_col)
# Do NOT assign gpio_out[15:8] to physical pins here - would cause conflicts!

# I/O Standard for LED GPIO bits only
...
# GPIO Input [7:0] -> ZedBoard 8개 스위치 (SW0-SW7) - Bank 34/35 (J18 점퍼로 2.5V 설정)
# 주의: Block Design에서 gpio_in은 8비트만 정의되어 있음
...
# I/O 표준 설정 - Bank 33은 1.8V로 설정됨 (주의!)
# 참고: ZedBoard Bank 33의 실제 전압은 보드 설정에 따라 결정됨
# set_property IOSTANDARD LVCMOS33 [get_ports {gpio_out[*]}]
set_property IOSTANDARD LVCMOS25 [get_ports {gpio_in[*]}]

#--------------------------------------------------------
# 4x4 Matrix Keypad - JD PMOD Connector (Bank 13, 3.3V)
#--------------------------------------------------------
# JD PMOD 핀 배치:
#    +--+--+--+--+--+--+
#    |V |G |4 |3 |2 |1 | upper row
#    +--+--+--+--+--+--+
#    |V |G |10|9 |8 |7 | lower row
#  --+--+--+--+--+--+--+--
#
# Keypad 연결:
#   - Col[0] -> JD1 (upper pin 1)
#   - Col[1] -> JD2 (upper pin 2)
#   - Col[2] -> JD3 (upper pin 3)
#   - Col[3] -> JD4 (upper pin 4)
#   - Row[0] -> JD7 (lower pin 1)
#   - Row[1] -> JD8 (lower pin 2)
#   - Row[2] -> JD9 (lower pin 3)
#   - Row[3] -> JD10 (lower pin 4)
#
# Column 출력 핀 (Column을 LOW로 설정하여 스캔)
# 주의: ZedBoard JD PMOD 핀 할당 (Bank 13, 3.3V)
...
# Row 입력 핀 (Pull-up으로 기본 HIGH, 버튼 누르면 LOW)
...
#
# I2C 연결:
#   - SDA (Data) -> JB1 (JB1_P, W12)
#   - SCL (Clock) -> JB2 (JB1_N, W11)
#
# 주의사항:
...
#   - GND        → JC 핀 5 (PMOD GND)
#   - SCK        → JC1 (JC1_N, AB6) → spi_sck
#   - MOSI/SDA   → JC2 (JC1_P, AB7) → spi_mosi
#   - MISO       → JC3 (JC2_P, AA4) → spi_miso
#   - CS/NSS     → JC4 (JC2_N, AB4) → spi_cs_n
#   - RST        → JC7 (JC3_P, T6) → spi_rst
#   - IRQ        → (선택사항, 연결 안 해도 됨)
#
# 주의사항:
